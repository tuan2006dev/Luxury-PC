/* ======================================================
   LUXURY PC - Profile Security Tab Script (security.js)
   ====================================================== */
...
function changePassword(e) {
    ...
    if (!oldPw || !newPw || !confirmPw) {
        toast('❗ Vui lòng nhập đầy đủ các ô mật khẩu.');
        return;
    }
    if (newPw !== confirmPw) {
    ...
    fetch('/api/profile/change-password', {
        ...
        body: JSON.stringify({ currentPassword: oldPw, newPassword: newPw, confirmPassword: confirmPw })
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
            ...
            toast('⚠️ ' + (data.message || 'Lỗi khi đổi mật khẩu.'));
        }
    })
    .catch(err => {
        console.error('Change password error:', err);
        toast('⚠️ Lỗi kết nối khi đổi mật khẩu.');
    });
}
        ...
        if (data.success && data.data) {
            ...
            listContainer.innerHTML = data.data.map(us => {
                ...
                const currentBadge = us.isCurrent
                    ...
                    : '<span style="font-size: 0.7rem; color: #10b981; border: 1px solid #10b981; padding: 2px 6px; border-radius: 3px; font-weight: 500;">Đang hoạt động</span>';

                const actionBtn = us.isCurrent
                    ? ''
                    : `<button class="btn-sec" style="border-color:#ef4444; color:#ef4444; font-size:0.75rem; padding:4px 10px; white-space:nowrap;" onclick="revokeSession('${us.sessionId}')">Đăng xuất</button>`;
                ...
                return `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; background: rgba(255,255,255,0.02); border: 1px solid #e2e8f0; border-radius: 6px; gap: 10px;">
            ...
            listContainer.innerHTML = '<div style="text-align:center; padding: 1.5rem; color: #ef4444; font-size: 0.85rem;">Không thể tải danh sách phiên.</div>';
        }
    })
    .catch(err => {
        console.error('Load sessions error:', err);
        listContainer.innerHTML = '<div style="text-align:center; padding: 1.5rem; color: #ef4444; font-size: 0.85rem;">Lỗi kết nối máy chủ.</div>';
    });
}
